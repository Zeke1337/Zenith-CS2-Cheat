...
bool GetPlayer(HANDLE hProc,uintptr_t moduleBase,  Player& player)
{
    ...
    uintptr_t playerController = Memory::RPM<uintptr_t>(hProc, (moduleBase + 0x19B3DE8));
...
void GetEntity(HANDLE hProc, Client& client, Player& player,Entity& entity,  int playerIndex, Config& cnf) {
    ...
    //entity.sceneNode = Memory::RPM<uintptr_t>(hProc, entity.baseAdress + dumper::schemas::client_dll::CSkeletonInstance::m_modelState + 0x80);
    ...
    //entity.NeckOrigin = Memory::RPM<Vec3>(hProc, (entity.sceneNode + entity.BoneId::head * 32));
